    ...
    public static function getListVisible()
    {
        ...
        foreach ($list as  $menu) {
            ...
            if(count($admin->schools) > 0) {
                if($admin->schools[0]->school_type == 6 && $menu->uri == 'admin::routing/admin.school.report.pl04') continue;
                if($admin->schools[0]->school_type != 6 && $menu->uri == 'admin::routing/admin.school.report.pl03') continue;
            }

            if ((!count($allPermissionsMenuAllow)  && !count($allRolesMenuAllow))
            || \Admin::user()->inRoles($allRolesMenuAllow)
