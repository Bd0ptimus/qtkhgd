...
class StaffDeclaration extends BaseExport implements WithStyles, WithTitle
{
    protected $school;
    protected $data;
    ...
    public function view(): View
    {
        ...
        $tableName = "DANH SÁCH KHAI BÁO Y TẾ NHÂN VIÊN";
        $extraInformation = 'Ngày ' . Carbon::now()->format('d/m/Y');

        $data = [];
        if($this->type == 'full') $data = $this->school->staffs;
        elseif($this->type == 'special') {
            foreach($this->school->staffs as $item) {
                if(count($item->medicalDeclarations->all()) > 0) array_push($data, $item);
            }
        }

        return view('exports.medical_declaration.staff_declaration', [
    ...
    {
        return 'Khai báo y tế nhân viên';
    }
}