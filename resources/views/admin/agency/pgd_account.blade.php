...
@section('main')
    @php
        ...
        $title = 'Danh sách các tài khoản Phòng Giáo dục và Đào tạo';
        $breadcrumbs = [
                                ...
                                <table class="table zero-configuration">
                                    ...
                                    @foreach($users as $key => $user)
                                        <tr>
                                            ...
                                            <td>
                                                @if(in_array($user->roles->first()->id, [ROLE_CV_PHONG_ID]))
                                                    <a type="button" id="create_sgd_account" name="001"
                                                       class="btn btn-flat btn-info waves-effect waves-light"
                                                       href="{{route('district.manage.specialist_school', ['districtId' => $district->id, 'provinceId' => $provinceId, 'specialistId' => $user->id])}}">
                                                        <i class="fa fa-bars" aria-hidden="true"></i> DS Trường đang phụ trách
                                                    </a>
                                                @endif
                                            </td>
                                        </tr>
