...
$breadcrumbs = [['name' => trans('admin.home'), 'link' => route('admin.home')], ['name' => $title_description ?? 'Danh sách phiếu bài tập về nhà']];
@endphp
@section('title', $title_description ?? 'Danh sách phiếu bài tập về nhà')
                    ...
                    <div class="card-body card-dashboard form-group form-filter">
                        <div class='row align-items-center'>
                            ...
                            <div class="col-sm-4 d-flex justify-content-end">
                                <div class="d-flex">
                                    <input type="text" name="search" id="search" placeholder="nhập tên bài giảng"
                                        value="{{ old('search', $search ?? '') }}" class="form-control search" />
                                    <button type="button" style="width: 50%;"
                                        class="btn btn-primary ml-1 input-search">{{ trans('admin.search') }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-content">
                        <div class="card-body card-dashboard">
                            <div class="d-flex">
                                <div class="btn-group text-nowrap">
                                    <a type="button" @if (!$permission) hidden @endif
                                        ...
                                        href="{{ route('homework_sheet.create') }}">
                            ...
                            <div class="table-responsive">
                                <table class="table" id="homework-sheet">
                                    <thead>
                                        <tr>
                                            <th scope="col">STT</th>
                                            <th scope="col">Tên phiếu bài tập</th>
                                    ...
                                    <tbody>
                                        @foreach ($homeworkSheets as $key => $homeworkSheet)
                                            <tr>
                                                ...
                                                <td>{{ $homeworkSheet->name }}</td>
                                                <td>{{ $homeworkSheet->assemblage }}</td>
                                                <td>{{ mapGradeName($homeworkSheet->grade) }}</td>
                                                <td>{{ $homeworkSheet->subject->name }}</td>
                                                <!--  <td class="homework-sheet-content"><p>{!! $homeworkSheet->content !!}</p></td> -->
                                                <td>
                                                    <a style="margin-top: 3px" href="#"
                                                        class="btn btn-flat btn-success btn-datatable" data-toggle="modal"
                                                        data-target="#modalContent{{ $homeworkSheet->id }}">
                                                    ...
                                                    <a type="button"
                                                        ...
                                                        href="{{ route('homework_sheet.edit', ['id' => $homeworkSheet->id]) }}">
                                                        <i class="fa fa-pencil" aria-hidden="true"></i>Sửa
                                                    </a>
                                                    <a type="button"
                                                        class="main-action btn btn-flat btn-danger delete-item btn-datatable"
                                                        @if (!$permission) hidden @endif
                                                        data-url="{{ route('homework_sheet.delete', ['id' => $homeworkSheet->id]) }}"
                                                        href="#">
                                                    ...
                                                    <a type="button"
                                                        ...
                                                        target="_blank"
                                                        href="{{ route('homework_sheet.download', ['id' => $homeworkSheet->id]) }}">
                                                        <span title="Download"><i class="fa fa-download"
                                                                aria-hidden="true"></i> Download</span>
                                                    </a>
                                                </td>
                                            </tr>
                                        @endforeach
                                ...
                                <div class="d-flex justify-content-end">
                                  {{ $homeworkSheets->appends(request()->query())->links()}}                                   
                                </div>
                            </div>

                            @include('admin.homework_sheet.modal_contents')
                        </div>
                    </div>
                </div>
            </div>
...
@section('page-script')
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $('#homework-sheet').on('click', '.delete-item', function(e) {
                e.preventDefault();
                let confirmDelete = confirm('Bạn có chắc chắn muốn xoá bài giảng này?');
                if (confirmDelete) {
                    let element = $(this);
                    $.ajax({
            ...
            function updateParameter(urlCurrent, key, value) {
                ...
                search_params.set('page', '1');//set page fetch data
