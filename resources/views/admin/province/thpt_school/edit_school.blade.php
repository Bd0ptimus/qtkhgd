                ...
                <div class="card">
                    <div class="card-content">
                        <div class="card-body card-dashboard">
                            <form method="POST"
                                  action="{{ route('admin.province.update_thpt_school', ['id' => $province->id, 'school_id' => $school->id]) }}"
                                  class="form">
                                @csrf
                                <div class="form-body">
                                    <div class="row">
                                                ...
                                                <select class="custom-select form-control required select2 filter-district"
                                                    ...
                                                    @foreach($province->districts as $district)
                                                        <option value="{{ $district->id }}"
                                                                @if($district->id == $selectedDistrict->id) selected @endif>{!! $district->name !!}</option>
                                                    @endforeach
                                                </select>
                                            ...
                                            <div class="form-group{{ $errors->has('ward_id') ? ' error' : '' }}">
                                                ...
                                                <select class="custom-select form-control required select2"
                                                    ...
                                                    @foreach($wards as $ward)
                                                        <option {{ strval($ward->id) === strval(old('ward',$school->ward_id)) ? 'selected' : '' }} value="{{ $ward->id }}">{{$ward->name}}</option>
                                                    @endforeach
                                                </select>
                                        ...
                                        <div class="col-md-6 col-12">
                                            <div class="form-group{{ $errors->has('school_address') ? ' error' : '' }}">
                                                ...
                                                <input type="text" class="form-control"
                                                       ...
                                                       value="{{ old('school_address', $school->school_address) }}"
                                                >
                                            ...
                                            <div class="form-group{{ $errors->has('school_type') ? ' error' : '' }}">
                                                ...
                                                <select class="custom-select form-control required select2"
                                                    ...
                                                    @foreach($data['school_type'] as $value => $label)
                                                        <option {{ strval($value) === strval(old('school_type', $school->school_type)) ? 'selected' : '' }} value="{{ $value }}">{{$label}}</option>
                                                    @endforeach
                                                </select>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('.filter-district').on('change', function () {
                var url = "{!! route('admin.province.edit_thpt_school', ['id' => $province->id, 'school_id' => $school->id]) !!}";
