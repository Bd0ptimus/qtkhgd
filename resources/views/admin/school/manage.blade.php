  ...
  <div class="card">
    ...
    <div class="card-content">
      ...
      <div class="swiper-centered-slides-2 swiper-container p-1">
          <div class="swiper-wrapper">
            <div class="swiper-slide rounded swiper-shadow py-1 px-1 d-flex">
                <div class="swiper-text"><a class="main-action" href="{{route('admin.school.chuanhoa', ['id' => $school->id])}}">Chuẩn hoá dữ liệu</a></div>
            </div>
        ...
        <div class="swiper-centered-slides-2 swiper-container p-1">
          <div class="swiper-wrapper">
            ...
            <div class="swiper-slide rounded swiper-shadow py-1 px-1 d-flex">
              <div class="swiper-text"><a target="_blank"href="{{route('admin.school.parent_accounts', ['id' => $school->id])}}">Danh sách phụ huynh</a></div>
            </div>
          </div>
        </div>
      </div>
          ...
          <div class="swiper-wrapper">
            ...
            <div class="swiper-slide rounded py-1 px-1 d-flex">
                <div class="swiper-text" type="button" id="subject-management">
                  ...
                  <a target="_blank" href="#">Quản lý chủ đề </a>
              @else
          ...
          <div class="swiper-wrapper">
            ...
            <div class="swiper-slide rounded swiper-shadow py-1 px-1 d-flex">
               @if(count($school->staffs) > 0)
                ...
                <div class="swiper-text"><a target="_blank"href="{{route('admin.school.staff.all_staff_plans', ['id' => $school->id])}}">Kế hoạch giáo dục của giáo viên</a></div>
              @else
...
<script type="text/javascript">
    $(document).ready(function(){
     ...
     var subjects = '' ;
     if(<?= $school->school_type ?> == <?= SCHOOL_MN ?> ){
      nameset = "Danh sách chủ đề";
      subjects = "Thống kê chủ đề theo khối";
     }else{
      nameset = "Danh sách môn học";
      subjects = "Thống kê môn học theo khối";
     }
    $.contextMenu({
        ...
        items: {
       "{{route('school.subject.index', ['id' => $school->id])}}": { name: nameset},
       "{{route('school.subject.subject_by_grade', ['id' => $school->id])}}": { name: subjects },
      }
    ...
    $(document).ready(function(){
     var nameset = '' ;
     if(<?= $school->school_type ?> == <?= SCHOOL_MN ?> ){
      nameset = "Phân công chuyên môn";
     }else{
      nameset = "Phân công phụ trách môn học";
     }
    $.contextMenu({
        selector: '#assignment',
        trigger: "left",
        ...
        "{{route('school.teaching_assignment.class_subjects', ['id' => $school->id])}}": { name: nameset },
      }
    });
  });
