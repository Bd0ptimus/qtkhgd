...
@section('main')
    @php 
        $title = "Kế hoạch giáo dục của  $school->school_name";
        $breadcrumbs = [
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <div class="card-title">Danh sách giáo viên 
                            <select class="btn btn-outline-success btn-datatable select-teacher">   
                                <option value="">Tất cả</option>
                                @foreach($school->staffs as $teacher)
                                    <option data-href="{{ route('school.staff.plans', ['school_id' => $school->id, 'staffId' => $teacher->id])}}" >{{ $teacher->fullname }}</option>
                                @endforeach
                            </select>
                    </div>
                    </div>
                        ...
                        <div class="card-body card-dashboard">
                            <div class="table-responsive">
                                <table class="table zero-configuration" id="shool-regular-group">
                                    ...
                                    <tbody>
                                    @foreach($staff as $key => $teacherPlan)
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <a style="margin-top: 3px" type="button" 
                                                    ...
                                                    href="{{ route('school.staff.plan.edit', ['school_id' => $school->id, 'staffId' => $teacherPlan->staff->id, 'rgId' => $teacherPlan->regularGroup->id, 'planId' => $teacherPlan->id]) }}">
                                                    <i class="fa fa-eye" aria-hidden="true"></i>Xem
                                                </a>
                                                <a style="margin-top: 3px" type="button" 
                                                    name=""
                                                    class="btn btn-flat btn-warning btn-datatable"
                                                    href="{{ route('school.staff.plan.download', ['school_id' => $school->id, 'staffId' => $teacherPlan->staff->id, 'rgId' => $teacherPlan->regularGroup->id, 'planId' => $teacherPlan->id]) }}">
                                                ...
                                                @if( $teacherPlan->status == PLAN_PENDING)
                                                    ...
                                                    <a style="margin-top: 3px" type="button"
                                                        ...
                                                        data-url="{{ route('school.staff.plan.delete', ['school_id' => $school->id, 'staffId' => $teacherPlan->staff->id, 'rgId' => $teacherPlan->regularGroup->id, 'planId' => $teacherPlan->id]) }}" 
                                                        href="#">
                                                ...
                                                @if($teacherPlan->status == PLAN_APPROVED)
                                                    <a style="margin-top: 3px" type="button" 
                                                        name=""
                                                        class="btn btn-flat btn-info btn-datatable"
                                                        href="{{ route('school.staff.teacher_lesson.index', ['school_id' => $school->id, 'staffId' => $teacherPlan->staff->id,'planId' => $teacherPlan->id]) }}">
