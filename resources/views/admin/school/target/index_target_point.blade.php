            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Danh sách chỉ tiêu năm học của trường.</h4>
                    </div>
                        ...
                        <div class="card-body card-dashboard">
                            ...
                            <div class="table-responsive">
                                <table class="table zero-configuration" id="shool-subject">
                                    ...
                                    @foreach($targetPoints as $key=>$point)
                                        <tr>
                                            <td scope="row" class="font-weight-bold">{{$key+1}}</td>
                                            <td class="font-weight-bold">{{ $point->content}}</td>
                                            <td><pre>{{ $point->teacherClass->class_name ?? ''}}</pre></td>
                                            <td><pre>{{ $point->teacherSubject->name ?? ''}}</pre></td>
                                            <td>{{ $point->result }} %</td>
                                            <td>{{ $point->index_point }} %</td>
                                            <td>{{ $point->final_point }} %</td>
                                            <td>
                                                <a style="margin-top: 3px; text-decoration: none; color:white;" type="button" id="self-result-button"
                                                    ...
                                                    data-url="{{ route('school.target.result.point', ['id' => $school->id, 'pointId' => $point->id]) }}">
