            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="card-title">Kế hoạch tổ chuyên môn {{ $regularGroup->name }}</h4>
                    </div>
                        ...
                        <div class="card-body card-dashboard">
                            <div class="row" style="margin-bottom: 10px">
                                <div class="col-sm-12">
                                    ...
                                    @foreach($groups as $group)
                                        <a class="btn btn-datatable {{ $group->id == $regularGroup->id ? 'btn-success' : 'btn-outline-success' }}" href="{{route('school.regular_group.plan.index', ['id' => $school->id, 'rgId' => $group->id])}}">{{ $group->name }}</a>
                                    @endforeach
                                </div>
                            </div>
                            <div class="row">
                                <div class="col-md-10">
                                    <form class="" method="GET" action="{{ route('school.regular_group.plan.index', ['id' => $school->id, 'rgId' => $rgId]) }}">
                                        <div class="row">
                                            <div class="col-sm-4">
                                    ...
                                    <div class="col-md-2 float-right">
                                        <div class="btn-group text-nowrap">
                                            <a type="button" class="btn btn-flat btn-success"
                                                href="{{ route('school.regular_group.plan.create', ['id' => $school->id, 'rgId' => $rgId]) }}">
                            ...
                            <div class="table-responsive">
                                <table class="table zero-configuration" id="shool-regular-group">
                                    ...
                                    <tbody>
                                    @foreach($regularGroup->groupPlans as $key => $groupPlan)
                                        @php if( $groupPlan->status != PLAN_APPROVED && !$canManage) continue; @endphp
                                        <tr>
                                            ...
                                            <td>
                                                <a style="margin-top: 3px" type="button" 
                                                    ...
                                                    href="{{ route('school.regular_group.plan.edit', ['id' => $school->id, 'rgId' => $rgId, 'planId' => $groupPlan->id]) }}">
                                                ...
                                                <a style="margin-top: 3px" type="button" 
                                                    ...
                                                    href="{{ route('school.regular_group.plan.download', ['id' => $school->id, 'rgId' => $rgId, 'planId' => $groupPlan->id]) }}">
                                                ...
                                                @if($canManage)
                                                    ...
                                                    @if($groupPlan->status == PLAN_PENDING)
                                                    <a style="margin-top: 3px" type="button" 
                                                        name=""
                                                        class="btn btn-flat btn-info"
                                                        href="{{ route('school.regular_group.plan.submit', ['id' => $school->id,'rgId' => $rgId, 'planId' => $groupPlan->id]) }}">
                                                        <i class="fa fa-send" aria-hidden="true"></i>Gửi hiệu trưởng duyệt
                                                    </a>
                                                    <a style="margin-top: 3px" type="button"
                                                        class="btn btn-flat btn-danger delete-item btn-datatable"
                                                        data-url="{{ route('school.regular_group.plan.delete', ['id' => $school->id, 'rgId' => $rgId, 'planId' => $groupPlan->id]) }}" 
                                                        href="#">
