                            ...
                            <div class="table-responsive">
                                <table class="table zero-configuration timetable" id="shool-regular-group">
                                    ...
                                    @foreach($school->classes as $key => $class)
                                        <tr>
                                            <td rowspan="5">{{ $class->class_name }}</td>
                                            <td>1</td>
                                            @foreach(['mon', 'tue', 'wed', 'thu', 'fri', 'sat'] as $date)
                                                ...
                                                @else
                                                <td>
                                                    @php $classSubject = isset($lessons[$class->id][$date.'_1']) ? $lessons[$class->id][$date.'_1']->classSubject : null; @endphp
                                                    @if($classSubject)
                                                    {{ $classSubject->subject->name }} - <a href="#">{{$classSubject->staff->fullname }}</a>
                                                    @endif
                                                </td>
                                                @endif
                                            @endforeach
