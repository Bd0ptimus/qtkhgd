...
@php
    ...
    $tableHeadings = [
        ...
        ['headerName' => 'Phường/xã', 'field' => 'ward'],
                        ...
                        <div class="card-body card-dashboard">
                            ...
                            <div class="btn-group pull-right " style="margin-right: 5px">
                                <div class="btn">
                                    ...
                                    href="{{ route('ward.manage.school_list.export', ['id' => $ward->id]).$query_string }}">
                                        Xuất excel
                                    </a>
                                </div>
                            </div>
                            <div class="form-group   {{ $errors->has('pay_type') ? ' has-error' : '' }}">
                                ...
                                <form method="GET" action="" class="form row">
                                    <div class="col-md-3">
                                        <label>Cấp</label>
                                        <select  class="form-control parent select2" style="width: 100%;"
                                                name="school_type">
                                            <option value=""><a href="#">Tất cả</a></option>
                                            @foreach ($school_types as $key => $value)
                                                <option value="{{ $key}}"
                                                {{  intval($key) == intval($school_type) ? 'selected' : '' }}>{!! $value !!}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <input type="hidden" name="provinceId" value="{{ $_GET['provinceId'] ?? null }}">
                                    <input type="hidden" name="districtId" value="{{ $_GET['districtId'] ?? null }}">
