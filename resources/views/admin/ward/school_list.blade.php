...
@php
    $title = 'Danh sách các đơn vị trường học '.$ward->name. ' - ' .$ward->district->name. ' - ' .$ward->district->province->name;
    $breadcrumbs = [
        ['name' => $title],
    ];
    $tableHeadings = [
    ...
    <script>
        @php
            ...
            foreach($schools as $index => $school) {
                ...
                $data['students'] = count($school->students);
                $data['action'] = [
                    'school' => route('admin.school.manage',['id' => $school->id]),
                    'route_edit' => route('admin.agency.districts.edit_school',['id' => $school->ward_id, 'school_id' => $school->id]),
                    'id' => $school->id
                ];
                $rowData[] = $data;
            }
